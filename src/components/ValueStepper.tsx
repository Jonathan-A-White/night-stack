import { useCallback, useEffect, useRef } from 'react';

interface ValueStepperProps {
  value: number;
  onChange: (next: number) => void;
  /** Step in canonical units. */
  step: number;
  /** Round a candidate value to display precision (canonical units). */
  round: (v: number) => number;
  format: (v: number) => string;
  decreaseLabel: string;
  increaseLabel: string;
  label?: string;
  helpText?: string;
}

// Long-press acceleration profile (ms between repeats).
const PRESS_INITIAL_DELAY_MS = 350;
const PRESS_MIN_INTERVAL_MS = 30;
const PRESS_RAMP_FACTOR = 0.85;
const TURBO_AFTER_REPEATS = 20;
const TURBO_STEP_MULTIPLIER = 5;

/**
 * Generic +/- stepper with long-press acceleration. `WeightStepper` and
 * `NeckStepper` are thin wrappers that supply units and formatting.
 */
export function ValueStepper({
  value,
  onChange,
  step,
  round,
  format,
  decreaseLabel,
  increaseLabel,
  label,
  helpText,
}: ValueStepperProps) {
  const valueRef = useRef(value);
  valueRef.current = value;

  const timerRef = useRef<number | null>(null);
  const repeatCountRef = useRef(0);
  const intervalRef = useRef(PRESS_INITIAL_DELAY_MS);

  const clearTimer = useCallback(() => {
    if (timerRef.current !== null) {
      window.clearTimeout(timerRef.current);
      timerRef.current = null;
    }
    repeatCountRef.current = 0;
    intervalRef.current = PRESS_INITIAL_DELAY_MS;
  }, []);

  useEffect(() => {
    return () => clearTimer();
  }, [clearTimer]);

  const applyStep = useCallback(
    (direction: 1 | -1) => {
      const turbo = repeatCountRef.current >= TURBO_AFTER_REPEATS;
      const s = step * (turbo ? TURBO_STEP_MULTIPLIER : 1);
      onChange(round(valueRef.current + direction * s));
    },
    [step, round, onChange],
  );

  const startPress = useCallback(
    (direction: 1 | -1) => {
      clearTimer();
      applyStep(direction);
      const tick = () => {
        applyStep(direction);
        repeatCountRef.current += 1;
        intervalRef.current = Math.max(PRESS_MIN_INTERVAL_MS, intervalRef.current * PRESS_RAMP_FACTOR);
        timerRef.current = window.setTimeout(tick, intervalRef.current);
      };
      timerRef.current = window.setTimeout(tick, PRESS_INITIAL_DELAY_MS);
    },
    [applyStep, clearTimer],
  );

  const endPress = useCallback(() => clearTimer(), [clearTimer]);

  const handlePointerDown = (direction: 1 | -1) => (e: React.PointerEvent<HTMLButtonElement>) => {
    e.currentTarget.setPointerCapture?.(e.pointerId);
    startPress(direction);
  };

  const handlePointerUp = (e: React.PointerEvent<HTMLButtonElement>) => {
    e.currentTarget.releasePointerCapture?.(e.pointerId);
    endPress();
  };

  return (
    <div className="weight-stepper">
      {label && <div className="form-label">{label}</div>}
      <div className="weight-stepper-row">
        <button
          type="button"
          className="btn btn-secondary weight-stepper-btn"
          aria-label={decreaseLabel}
          onPointerDown={handlePointerDown(-1)}
          onPointerUp={handlePointerUp}
          onPointerCancel={endPress}
          onPointerLeave={endPress}
          onContextMenu={(e) => e.preventDefault()}
        >
          &minus;
        </button>
        <div className="weight-stepper-value">{format(value)}</div>
        <button
          type="button"
          className="btn btn-secondary weight-stepper-btn"
          aria-label={increaseLabel}
          onPointerDown={handlePointerDown(1)}
          onPointerUp={handlePointerUp}
          onPointerCancel={endPress}
          onPointerLeave={endPress}
          onContextMenu={(e) => e.preventDefault()}
        >
          +
        </button>
      </div>
      {helpText && (
        <div className="text-secondary text-sm mt-8" style={{ textAlign: 'center' }}>
          {helpText}
        </div>
      )}
    </div>
  );
}
