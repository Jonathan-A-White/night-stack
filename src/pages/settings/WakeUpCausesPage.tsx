...
export { WakeUpCausesPage };

export default function WakeUpCausesPage() {
  const items = useLiveQuery(
    () => db.wakeUpCauses.orderBy('sortOrder').toArray()
  );
  ...
  const handleToggleActive = async (id: string, isActive: boolean) => {
    await db.wakeUpCauses.update(id, { isActive: !isActive });
  };
  ...
  const saveEdit = async () => {
    ...
    await db.wakeUpCauses.update(editingId, { label: editLabel.trim() });
    setEditingId(null);
  };
      ...
      {showAdd ? (
        <div className="card">
          <div className="form-group">
            <label className="form-label">Cause Label</label>
            <input
              className="form-input"
              value={newLabel}
